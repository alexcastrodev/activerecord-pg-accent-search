...
UPPER_CASE = 'ABCDEFGHIJKLMNOPQRSTUVWXYZÀÁÂÃÄÅÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÇÑÝ'
LOWER_CASE = 'abcdefghijklmnopqrstuvwxyzàáâãäåèéêëìíîïòóôõöùúûüçñý'

class Product < ActiveRecord::Base
  scope :search_by_name, ->(term) {
    where(
      "translate(name, '#{UPPER_CASE}', '#{LOWER_CASE}') = translate(?, '#{UPPER_CASE}', '#{LOWER_CASE}')",
      term
    )
  }
end

class TranslateSearchTest < Minitest::Test
  def setup
    Product.delete_all
